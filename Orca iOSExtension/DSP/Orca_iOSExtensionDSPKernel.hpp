//
//  Orca_iOSExtensionDSPKernel.hpp
//  Orca iOSExtension
//
...
#import "Orca_iOSExtension-Swift.h"
#import "Orca_iOSExtensionParameterAddresses.h"

constexpr uint16_t kMaxVelocity = std::numeric_limits<std::uint16_t>::max();

/*
 Orca_iOSExtensionDSPKernel
 As a non-ObjC class, this is safe to use from render thread.
 */
class Orca_iOSExtensionDSPKernel {
public:
    ...
    // Add a case for each parameter in Orca_iOSExtensionParameterAddresses.h
    void setParameter(AUParameterAddress address, AUValue value) {
        switch (address) {
            case Orca_iOSExtensionParameterAddress::midiNoteNumber:
                mNextNoteToSend = (uint8_t)value;
                break;
            case Orca_iOSExtensionParameterAddress::sendNote:
    ...
    void handleMIDIEventList(AUEventSampleTime now, AUMIDIEventList const* midiEvent) {
        /*
         ...
         auto thisObject = static_cast<Orca_iOSExtensionDSPKernel *>(context);
